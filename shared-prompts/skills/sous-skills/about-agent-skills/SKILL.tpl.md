...
skill when...`. Cross-references to other skills must be equally mandatory. In a
skill body, write the requirement so it binds whichever agent executes ("The agent
performing this work MUST load `x`"), not just the main session; per the Sub-Agent
Delegation memory, delegated sub-agents start with fresh context and must load the
skills themselves.
