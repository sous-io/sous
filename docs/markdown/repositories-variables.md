...
it are attributed to a pseudo-recipe named after the file, so they generate the usual names.

!> A definition's `validate.pattern` is a regular expression published by someone else, so sous never runs
one on the thread waiting for your answer: the match happens on a worker under a fixed budget of 100
milliseconds. A pattern that exceeds it fails validation naming the pattern and the recipe, rather than
blaming your answer. Only the match is budgeted; compiling one cannot run away.
...
The keys that do anything are named by the legend under the input line. Enter accepts what is typed, or the
default when nothing is; a question with no default drops that half and shows Tab alone. A question answered
from a list names the arrow keys instead, and a yes-or-no question names the two letters.
...
**Changing the stored variable name** offers every rung the ladder looks up, with the one sous would use
already selected, plus a name you type yourself. A name the ladder would never look at is bound with a
[mapping record](#when-names-collide-mapping-records), so resolution still finds it.
...
"Discard changes" item joins it; returning prints the question again with the updated facts. When an answer
is finally entered, two lines say what was stored and where, and a secret is never printed:
...
Every run ends with the same three-part report: what was inherited from an answer already in scope, what was
stored and under which name in which file, and what was left unanswered and why.
...
Comments are output only. Sous never reads one back, so editing or deleting one changes nothing, and editing
the value line is a perfectly normal way to change an answer.
...
has exactly one. A record reads `"TEAM_API_URL": "sous-recipes:misc/stuff/apiUrl"`: the target is written
`namespace/recipe/variableName`, optionally qualified with a repository. Records live under the top-level
`varMappings` config key, so you may hand-write them in your primary config too.
...
`NAME` is a reference, resolved the way every sous command resolves one; anything larger than a variable asks
every question it publishes:
...
Matching is case-sensitive, and an environment variable name resolves when a definition declares it or when
one of the env files sets it. `--repo`, `--namespace` and `--var` say which kind of thing is meant, each
narrowing what the one before it left, so `sous vars ask --namespace workflow apiUrl` asks about that
namespace's `apiUrl` alone. When a name matches several things, sous lists them and asks; `--accept-first`
takes the first, and a run with no terminal fails naming that flag.

?> Bare `sous vars` is shorthand for `vars list`, and `sous vars <name>` for `vars show <name>`. A variable
named `list`, `show` or `ask` collides with the subcommand, so reach it the long way: `sous vars show ask`.
...
`sous vars ask --var qaScratchDir --answer qaScratchDir=/var/tmp/qa-review --dry-run`. Both flags work
identically on `sous subscription add`; see
...
[When sous cannot ask](repositories-consuming.md#when-sous-cannot-ask). Such a run fails on the unanswered
required variables, naming every environment variable that would satisfy each one:

```term
  Error: 5 variables still need answers, and there is no terminal to ask on.
...
Answers are environment variables, and a template renders your project's config variables. The bridge is one
`_env` entry in your config, naming the variable the answer is stored under (`sous vars show` prints that as
`stored-as`):
...
them in. Whether answers should be injected too is an open question, not settled behavior.
