 ...
 * ones it creates into the machine-written `conf.d/520-var-mappings.json`
...
import { stableJsonStringify } from "../repos/formats/common.js";
import {
...
export const VAR_MAPPINGS_LAYER_FILENAME = "520-var-mappings.json";

/** The comment sous writes at the top of the mapping record layer. */
export const VAR_MAPPINGS_COMMENT =
  "Written by sous. Each entry binds an environment variable to one recipe " +
...
/**
 * Records a mapping in the machine-written `conf.d/520-var-mappings.json`
 * layer, replacing the file wholesale with stable, sorted JSON so repeated
 * writes produce a minimal diff.
 *
 * The file is replaced rather than appended to because config layers merge
...
export function writeMappingRecord(
  ...
  const existing = readMappingRecords(filePath);
...
export function readMappingRecords(filePath: string): Record<string, string> {
  ...
  try {
    ...
    throw new ConfigError(
      `Could not read the variable mapping records at ${filePath}:\n` +
        `  ${(error as Error).message}\n` +
  ...
  if (block === null || typeof block !== "object" || Array.isArray(block)) {
    throw new ConfigError(
      `The variable mapping records at ${filePath} are malformed: 'varMappings' must ` +
