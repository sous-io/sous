...
describe("CompilationService", () => {
  ...
  // -------------------------------------------------------------------------
  // Unrendered .tpl. detection
  // -------------------------------------------------------------------------

  /**
   * A ".tpl." source written to an output with NO vars never reaches LiquidJS, so
   * its tags ship verbatim. That is almost always a config mistake, so the
   * compiler must warn loudly and name the offending file.
   *
   * entry.tpl.md → output with no `vars`
   * After compile: result is true (a warning, not a failure), a warning naming
   * the file was printed, and the output still contains the raw tag.
   */
  it("should warn and name the file when a .tpl. source is written with no vars", async () => {
    tmp = makeTmpDir();
    const srcFile = path.join(tmp.path, "thing.tpl.md");
    fs.writeFileSync(srcFile, "# Thing\n\n{{ projectName }}\n", "utf8");
    const destFile = path.join(tmp.path, "out", "thing.md");
    ...
    const compiler = makeCompiler();
    const result = await compiler.compile({
      targets: [{ rootInputPath: srcFile, outputs: [{ destinationFile: destFile }] }],
    });
    ...
    expect(output).toContain("WITHOUT BEING RENDERED");
    expect(output).toContain(srcFile);
    expect(output).toContain(destFile);

    // The tag really did survive into the output, which is what the warning is about.
    expect(fs.readFileSync(destFile, "utf8")).toContain("{{ projectName }}");
  });

  /**
   * With `strict: true`, the same situation must fail the compile rather than
   * only warning, so CI catches it.
   *
   * entry.tpl.md → output with no `vars`, strict: true
   * After compile: result is false.
   */
  it("should fail the compile for an unrendered .tpl. file when strict is set", async () => {
    tmp = makeTmpDir();
    const srcFile = path.join(tmp.path, "thing.tpl.md");
    fs.writeFileSync(srcFile, "# Thing\n\n{{ projectName }}\n", "utf8");
    const destFile = path.join(tmp.path, "out", "thing.md");
    ...
    const compiler = makeCompiler({ strict: true });
    const result = await compiler.compile({
      targets: [{ rootInputPath: srcFile, outputs: [{ destinationFile: destFile }] }],
    });
  ...
  /**
   * An empty `vars: {}` is enough to enable rendering, so it must NOT trigger the
   * warning — this is the documented fix and the shape existing configs use.
   *
   * entry.tpl.md → output with `vars: {}`
   * After compile: no "WITHOUT BEING RENDERED" warning; the tag was rendered away.
   */
  it("should not warn when the output has an empty vars object", async () => {
    tmp = makeTmpDir();
    const srcFile = path.join(tmp.path, "thing.tpl.md");
    fs.writeFileSync(srcFile, "# Thing\n\n{{ projectName }}\n", "utf8");
    const destFile = path.join(tmp.path, "out", "thing.md");
    ...
    const compiler = makeCompiler();
    const result = await compiler.compile({
      targets: [{ rootInputPath: srcFile, outputs: [{ destinationFile: destFile, vars: {} }] }],
    });

    logSpy.mockRestore();

    expect(result).toBe(true);
    expect(logged.join("\n")).not.toContain("WITHOUT BEING RENDERED");
    expect(fs.readFileSync(destFile, "utf8")).not.toContain("{{ projectName }}");
  });

  /**
   * A non-".tpl." source with no vars is a plain verbatim copy and must not warn —
   * that is the normal, intended path for files that are not templates.
   *
   * plain.md → output with no vars
   * After compile: no "WITHOUT BEING RENDERED" warning.
   */
  it("should not warn for a non-.tpl. source written with no vars", async () => {
    tmp = makeTmpDir();
    const srcFile = path.join(tmp.path, "plain.md");
    fs.writeFileSync(srcFile, "# Plain\n", "utf8");
    const destFile = path.join(tmp.path, "out", "plain.md");
  ...
  /**
   * An explicitly-undefined `includeSourceComments` must NOT be reported as an
   * error. The settings resolver always sets the key (to undefined when the config
   * omits it), so a presence-only check fired on every build and printed a bogus
   * "must be a boolean" error.
   *
   * compile({ includeSourceComments: undefined, targets: [...] });
   * // -> succeeds, no error logged
   */
  it("should not error when includeSourceComments is present but undefined", async () => {
    tmp = makeTmpDir();
    const fixtureDir = copyFixture("simple", tmp.path);
    const entryPoint = path.join(fixtureDir, "entry.md");
    const destFile = path.join(tmp.path, "out", "entry.md");
    ...
    const compiler = makeCompiler();
    const result = await compiler.compile({
      includeSourceComments: undefined,
      targets: [{ rootInputPath: entryPoint, outputs: [{ destinationFile: destFile }] }],
    });
    ...
    expect(result).toBe(true);
    expect(logged.join("\n")).not.toContain("must be a boolean");
  });

  /**
   * A non-boolean `includeSourceComments` should still be reported, since it is a
   * genuine config mistake.
   *
   * compile({ includeSourceComments: "yes", targets: [...] });
   * // -> logs "must be a boolean"
   */
  it("should error when includeSourceComments is a non-boolean value", async () => {
    tmp = makeTmpDir();
    const fixtureDir = copyFixture("simple", tmp.path);
    const entryPoint = path.join(fixtureDir, "entry.md");
    const destFile = path.join(tmp.path, "out", "entry.md");
    ...
    await compiler.compile({
      ...
      includeSourceComments: "yes" as any,
      targets: [{ rootInputPath: entryPoint, outputs: [{ destinationFile: destFile }] }],
    });
