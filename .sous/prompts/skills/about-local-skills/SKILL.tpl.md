...
description: >
  YOU MUST load this skill when working with skills in {{ toolMetaDirectory }}/skills/ in the sous project or when the user mentions 
...
Local skills are **compiled** into `{{ toolMetaDirectory }}/skills/` — but that is the output directory, not where you edit them.
...
- **Compiled output** goes to `{{ toolMetaDirectory }}/skills/<skill-name>/SKILL.md`
- Running `xcv build` compiles the templates and writes the output
- **Never edit files in `{{ toolMetaDirectory }}/skills/` directly** — they are overwritten on every build

Since the source templates are `.tpl.md` files, they are processed through LiquidJS at compile time. This
is why every local skill must include the `## Source for this Skill` footer with `{{ sousTemplatePath }}` —
