...
a source template to `.sous/prompts/skills/` and compiling it to `.claude/skills/` with `xcv build`.
...
Place the skill directory at `.sous/prompts/skills/<skill-name>/` — not `.claude/skills/`
...
must include the `## Source for this Skill` footer containing `{{ sousTemplatePath }}`.
...
- Source Path: /home/luke/Projects/puravida/infra/sous/.sous/prompts/skills/create-local-skill/SKILL.tpl.md
