...
The `` !`command` `` syntax runs a shell command before skill content is sent to the agent.
The output replaces the placeholder — the agent receives the rendered result, not the command.
...
Execution order: each `` !`command` `` runs first, output is inserted, then the agent sees
the fully-rendered prompt. This is preprocessing, not something the agent executes.
...
- Any custom agent defined in the tool's agents directory
...
Limit which tools the agent can use when a skill is active. Granted without per-use approval.
...
and reference them from the skill body so the agent knows when to load them.
